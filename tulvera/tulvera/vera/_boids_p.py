...
class BoidsP(Particles):
    ...
    def raster(self):
        ...
        for i,j in ti.ndrange((0, self._x),(0, self._y)):
            self.world[i, j] = ti.Vector([0,0,0])
        for i in range(self._n):
            ...
            for x in range(xi, xj):
                for y in range(yi, yj):
                    self.world[x, y] = ti.Vector([
                        ti.cast(self._vel[i][1]*255, ti.i32),
                        ti.cast(self._vel[i].norm()*255, ti.i32),
                        ti.cast(self._vel[i][0]*255, ti.i32)])
